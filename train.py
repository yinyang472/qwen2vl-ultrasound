...
def process_func(example):
    ...
    input_content = example["images"]
    output_content = example["report"].replace("\n","")
    ...
    inputs['image_grid_thw'] = torch.tensor([len(inputs['image_grid_thw']), inputs['image_grid_thw'][0][1], inputs['image_grid_thw'][0][2]])  # 由（1,h,w)变换为（h,w）
    # inputs['image_grid_thw'] = torch.tensor([1,20,20])  # 由（1,h,w)变换为（h,w）       是对的
